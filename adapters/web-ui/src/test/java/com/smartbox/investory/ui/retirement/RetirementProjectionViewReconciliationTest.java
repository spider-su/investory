...
class RetirementProjectionViewReconciliationTest {
  ...
  void cashFlowTimelineAndYearSummaryUseTheSameAuthoritativeYearValues() {
    ...
    var timelineView = PlanTimelineView.from(
        timeline, summaries, moneyByYear, assumptions, 2026, 2052, 2026);
    ...
    assertThat(timelineView.years().getFirst().incomeSources()).extracting(CashFlowFlowView::source)
        .containsExactly("Rents", "Bond cash income", "Bonds", "Equities");
    assertThat(timelineView.years().getFirst().fundingSources()).extracting(CashFlowFlowView::source)
