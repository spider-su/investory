...
public class ScenarioObservationService {
  ...
  public ScenarioObservationService(
      LongTermAssetAnnualSnapshotReader longTerm,
      LongTermAssetsFacade longTermAssets,
      PortfolioPerformanceQuery performance,
      Clock clock) {
  ...
  public Map<String, ScenarioObservation> load(Long portfolioId, PlanningTimeline timeline) {
    ...
    LongTermAssetAnnualSnapshotModel prior =
        longTerm.historicalAnnualSnapshot(portfolioId, today.getYear() - 1);
    ...
    BigDecimal returnRate =
        actual.returnPercentage() == null ? null : actual.returnPercentage().movePointLeft(2);
    result.put(
        "Equity return",
        returnRate == null
  ...
  private ScenarioObservation averageBondYield(Long portfolioId, LocalDate date) {
    var yields =
        longTermAssets.list(portfolioId, date).stream()
            .filter(asset -> asset.type() == LongTermAssetType.BOND)
            .map(
                com.smartbox.investory.longterm.application.model.LongTermAssetSummary
                    ::currentAnnualRate)
  ...
  private ScenarioObservation spendingGrowth(PlanningTimeline timeline) {
    ...
    if (elapsedYears > 1)
      rate = BigDecimal.valueOf(Math.pow(rate.doubleValue(), 1.0 / elapsedYears));
    return new ScenarioObservation(
  ...
  private static BigDecimal value(
      Map<PlanningMetric, com.smartbox.investory.retirement.planning.PlanningMetricValue> values,
  ...
  private ScenarioObservation growth(BigDecimal current, BigDecimal prior) {
    if (current == null || prior == null || prior.signum() == 0)
      return new ScenarioObservation(
          ...
          ScenarioAssumptionView.Availability.INSUFFICIENT_HISTORY);
    return new ScenarioObservation(
        current.divide(prior, 12, java.math.RoundingMode.HALF_UP).subtract(BigDecimal.ONE),
        "Observed annualized",
        "year over year",
        ScenarioAssumptionView.Availability.AVAILABLE);
  }
