...
class AiInvoiceRecognitionClient {
  ...
  AiInvoiceRecognitionClient(
      ...
      @Value("${app.openai.connect-timeout:PT10S}") Duration connectTimeout,
      @Value("${app.openai.read-timeout:PT60S}") Duration readTimeout) {
  ...
  AccountingInvoiceRecognitionService.RecognizedInvoice recognize(
    ...
    var httpClient = java.net.http.HttpClient.newBuilder().connectTimeout(connectTimeout).build();
  ...
  private AccountingInvoiceRecognitionService.RecognizedInvoice parseRecognizedInvoice(
    ...
    try {
      ...
      if (invoice.issueDate() == null) missing.add("issueDate");
      if (invoice.reference() == null) missing.add("reference");
      if (invoice.seller() == null) missing.add("seller");
      if (invoice.buyer() == null) missing.add("buyer");
      if (invoice.netAmount() == null) missing.add("netAmount");
      if (invoice.vatAmount() == null) missing.add("vatAmount");
      if (invoice.grossAmount() == null) missing.add("grossAmount");
      if (!missing.isEmpty()) {
        throw new IllegalStateException("AI result is missing required fields: " + missing);
      }
      return invoice;
  ...
  private LocalDate localDate(JsonNode node, String field) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Invalid AI field " + field + " value '" + value + "'", exception);
    }
  }
