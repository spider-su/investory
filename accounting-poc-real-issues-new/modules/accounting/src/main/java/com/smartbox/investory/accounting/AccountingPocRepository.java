...
public class AccountingPocRepository {
  ...
  public HealthContributionBasis healthContributionBasis(LocalDate period) {
    List<String> values =
        jdbcTemplate.queryForList(
            """
            SELECT basis
              FROM investory.accounting_health_contribution_basis
             WHERE profile_id = 1
               AND valid_from <= ?
    ...
    return values.isEmpty()
        ? HealthContributionBasis.HIGH
        : HealthContributionBasis.valueOf(values.getFirst());
  }
