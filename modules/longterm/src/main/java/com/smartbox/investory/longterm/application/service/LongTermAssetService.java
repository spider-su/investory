...
public class LongTermAssetService {
  ...
  public LongTermAssetEntity saveCashReserve(
    ...
    if (annualReturnRate != null) {
      saveOpenValuationPeriod(asset.getId(), effectiveFrom, annualReturnRate);
    } else closeValuationPeriods(asset.getId(), effectiveFrom);
    return asset;
  }
  ...
  public void saveExpectedPropertyGrowth(
    ...
    if (growthRate != null) {
      saveOpenValuationPeriod(assetId, effectiveFrom, growthRate);
    } else closeValuationPeriods(assetId, effectiveFrom);
  }
  ...
  private void closeValuationPeriods(Long id, LocalDate from) {
    for (LongTermAssetValuationPeriodEntity old :
      ...
      if (old.getValidFrom().isBefore(from)) {
        old.setValidTo(from.minusDays(1));
        valuations.save(old);
      } else if (old.getValidFrom().isAfter(from)) valuations.delete(old);
    }
  }
