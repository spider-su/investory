...
public class LongTermAssetCashFlowService {
  ...
  public LongTermAssetCashFlow add(Long portfolioId, Long assetId, LongTermAssetCashFlow flow) {
  ...
  public void saveRentalPeriod(
      ...
      LocalDate endDate,
      List<LongTermAssetCashFlow> current) {
    ...
    List<LongTermAssetCashFlow> all = cashFlows.findAllByAssetIdOrderByValidFrom(assetId);
  ...
  private void validate(LongTermAssetCashFlow flow) {
  ...
  private static void requireRealEstate(LongTermAsset asset) {
  ...
  private static boolean isExpense(com.smartbox.investory.longterm.api.CashFlowType type) {
    return switch (type) {
