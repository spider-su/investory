...
public final class CashFlowAggregationService {
  ...
  public Result aggregate(
    ...
    return aggregateInternal(
        period,
        replaceProjectedWithActual(flows == null ? List.of() : flows),
        includeActual,
        includeProjected);
  }
  ...
  private Result aggregateInternal(
    ...
    for (PlannedCashFlow flow : selected) {
      if (!applicable(flow, period) || !included(flow.source(), includeActual, includeProjected))
        continue;
  ...
  private static List<PlannedCashFlow> replaceProjectedWithActual(List<PlannedCashFlow> flows) {
    List<PlannedCashFlow> actual =
        flows.stream().filter(flow -> flow.source() == ProjectionSource.ACTUAL).toList();
    ...
    for (PlannedCashFlow flow : flows) {
      ...
      List<PlannedCashFlow> replacements =
          actual.stream().filter(candidate -> sameOccurrence(flow, candidate)).toList();
  ...
  private static boolean sameOccurrence(PlannedCashFlow projected, PlannedCashFlow actual) {
    if (!projected.id().equals(actual.id()) || projected.cadence() != actual.cadence())
      return false;
    ...
    LocalDate actualEnd =
        actual.effectiveTo() == null ? actual.effectiveFrom() : actual.effectiveTo();
  ...
  private static boolean applicable(PlannedCashFlow flow, Period period) {
    ...
    return switch (flow.cadence()) {
      case ONE_OFF -> {
        ...
        yield !event.isBefore(period.start())
            && !event.isAfter(period.end())
            && !start.isAfter(event)
            && !end.isBefore(event);
      }
      case ANNUAL -> !start.isAfter(period.end()) && !end.isBefore(period.start());
      case MONTHLY ->
          !YearMonth.from(start).isAfter(YearMonth.from(period.end()))
  ...
  public record Result(
    ...
    public BigDecimal periodIncome() {
      return actualIncome.add(projectedIncome);
    }

    public BigDecimal periodExpenses() {
      return actualExpenses.add(projectedExpenses);
    }

    public BigDecimal netCashFlow() {
      return periodIncome().subtract(periodExpenses());
    }

    public BigDecimal fundingGap() {
      return netCashFlow().negate().max(BigDecimal.ZERO);
    }

    public BigDecimal surplus() {
      return netCashFlow().max(BigDecimal.ZERO);
    }
