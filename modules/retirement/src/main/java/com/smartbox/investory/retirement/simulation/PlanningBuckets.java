...
public record PlanningBuckets(PlanningBucket cash, PlanningBucket bonds, PlanningBucket equities,
  ...
  public PlanningBuckets {
    ...
    rentalCashIncome = zeroIfNull(rentalCashIncome); realEstateGrowthRate = zeroIfNull(realEstateGrowthRate);
  }
  ...
  public static PlanningBuckets fromProfile(InvestmentProfile profile, BigDecimal equityYield,
      BigDecimal fallbackBondYield, int baselineYear) {
    return fromProfileWithBondYield(profile, equityYield,
        baseBondYield(profile, fallbackBondYield, baselineYear));
  }
  ...
  public static PlanningBuckets fromProfileWithBondYield(InvestmentProfile profile,
    ...
    if (!hasAllocation(profile, EconomicBucket.FIXED_INCOME)) bonds = profile.longTermAssets().stream()
        ...
        .map(a -> zeroIfNull(a.currentValue())).reduce(BigDecimal.ZERO, BigDecimal::add);
    BigDecimal equities = allocation(profile, EconomicBucket.EQUITY);
    if (!hasAllocation(profile, EconomicBucket.EQUITY)) equities = zeroIfNull(profile.investmentCapital());
    ...
    if (!hasAllocation(profile, EconomicBucket.REAL_ESTATE)) realEstate = profile.longTermAssets().stream()
        ...
        .map(a -> zeroIfNull(a.currentValue())).reduce(BigDecimal.ZERO, BigDecimal::add);
    return of(zeroIfNull(profile.retirementReserve()), bonds, equities, realEstate, zeroIfNull(bondYield),
        zeroIfNull(equityYield), bonds, profile.currentRentalIncome());
  }
  ...
  public static BigDecimal baseBondYield(
    ...
    return new FrozenBondCashFlowProjection().baseCapitalizedBondYield(
        profile, fallbackBondYield, baselineYear);
  }
  ...
  private static BigDecimal allocation(InvestmentProfile profile, EconomicBucket bucket) {
    return profile.allocations().stream().filter(a -> a.bucket() == bucket)
        .map(a -> zeroIfNull(a.value())).reduce(BigDecimal.ZERO, BigDecimal::add);
  }
