package com.smartbox.investory.retirement.preview;
...
public class PlanEditorPreviewService {
  ...
  public PlanEditorPreview preview(
    ...
    SimulationYear plannedIncomeYear =
        result.years().stream()
            .filter(
                row ->
                    row.year() > currentYear
                        && row.lifecyclePhase() == SimulationLifecyclePhase.RETIRED)
            .findFirst()
            .orElse(first);
    ...
    previewYears.addAll(
        result.years().stream()
            ...
            .map(row -> year(row, assumptions, displayCurrency))
            .toList());
    return new PlanEditorPreview(
        ...
        plannedIncomeYear == null
            ? null
            : displayCanonical(plannedIncomeYear.rentalIncome(), displayCurrency),
        plannedIncomeYear == null
            ? null
            : displayCanonical(plannedIncomeYear.bondIncome(), displayCurrency),
        plannedIncomeYear == null
            ? null
            : displayCanonical(plannedIncomeYear.funding().investmentReturn(), displayCurrency),
        plannedIncomeYear == null
            ? null
            : displayCanonical(
                plannedIncomeYear.funding().capitalizedBondReturn(), displayCurrency),
        plannedIncomeYear == null
            ? null
            : displayCanonical(plannedIncomeYear.pensionIncome(), displayCurrency),
        plannedIncomeYear == null
            ? null
            : displayCanonical(plannedIncomeYear.totalIncome(), displayCurrency),
        assumptions,
        result.failureAge(),
  ...
  private PreviewYear year(
      SimulationYear row, SimulationAssumptions assumptions, CurrencyType displayCurrency) {
    ...
    return new PreviewYear(
        ...
        assumptions.effectiveSpendingGrowthRate(),
        assumptions.expenseProfileFactorForCalendarYear(row.year()),
        ...
        displayCanonical(funding.investmentReturn(), displayCurrency),
        ...
        displayCanonical(funding.reserveEnd(), displayCurrency),
        displayCanonical(funding.longTermFunding(), displayCurrency),
        displayCanonical(funding.longTermCapitalEnd(), displayCurrency),
        displayCanonical(funding.investmentStart(), displayCurrency),
        displayCanonical(funding.investmentReturn(), displayCurrency),
        displayCanonical(funding.investmentWithdrawal(), displayCurrency),
        displayCanonical(funding.investmentEnd(), displayCurrency),
        displayCanonical(funding.unfunded(), displayCurrency),
        funding.unfunded().signum() > 0 ? "UNFUNDED" : "FUNDED");
  }
  ...
  private PreviewYear currentYear(
    ...
    return new PreviewYear(
        ...
        null,
        displayCanonical(currentIncomeTotal, displayCurrency),
        null,
        null,
        null,
        null,
  ...
  private PreviewYear historicalYear(
    ...
    BigDecimal totalIncome =
        rental == null && bond == null ? null : zeroIfNull(rental).add(zeroIfNull(bond));
    boolean retired = age >= assumptions.retirementAge();
    return new PreviewYear(
        year,
        age,
