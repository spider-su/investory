...
public final class SimulationAssumptionsPersistenceMapper {
  ...
  public static void write(
    ...
    RetirementFundingPolicy policy = assumptions.fundingPolicy();
    target.setFundingStrategy(policy.fundingStrategy());
    target.setExpenseProfile(serializeExpenseProfile(assumptions.expenseProfile()));
    target.setFundingOrder(
        policy.fundingOrder().stream()
            .map(SimulationAssumptionsPersistenceMapper::serializeFundingSource)
            .collect(Collectors.joining(",")));
    target.setSafeReserveYears(policy.reserveTargetYears());
    target.setEquityHarvestMinimumReturnRate(policy.equityHarvestThresholdRate());
    target.setEquityGainHarvestRate(policy.equityHarvestShare());
    target.setAllowEmergencyEquityWithdrawal(policy.allowEmergencyEquityWithdrawal());
