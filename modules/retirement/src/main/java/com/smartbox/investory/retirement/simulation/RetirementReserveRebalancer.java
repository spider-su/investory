...
public final class RetirementReserveRebalancer {
  ...
  public Result rebalance(
      BigDecimal reserveBeforeHarvest,
    ...
    BigDecimal shortfall =
        zeroIfNull(reserveTarget).subtract(zeroIfNull(reserveBeforeHarvest)).max(ZERO);
    ...
    BigDecimal harvest =
        gain.multiply(policy.investmentHarvestShare())
            .min(shortfall)
            .min(zeroIfNull(investment.endValue()).max(ZERO));
    ...
    var adjusted =
        new InvestmentAnnualProjectionApi.AnnualProjection(
            ...
            investment.externalContribution(),
            investment.annualReturnAmount(),
            investment.withdrawal(),
            investment.endValue().subtract(harvest).max(ZERO),
            investment.source());
    return new Result(adjusted, harvest);
  }

  public record Result(
      InvestmentAnnualProjectionApi.AnnualProjection investment, BigDecimal harvestToReserve) {}
}
