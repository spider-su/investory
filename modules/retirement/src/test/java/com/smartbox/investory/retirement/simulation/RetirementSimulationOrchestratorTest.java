...
class RetirementSimulationOrchestratorTest {
  ...
  void usesFixedFundingOrderAndCarriesProviderEndStates() {
    ...
    LongTermAnnualProjectionApi longTerm = longTerm(request -> {
      ...
      return new LongTermAnnualProjectionApi.PlanningProjection(request.year(),
          List.of(new LongTermAnnualProjectionApi.PlannedCashFlow("rent", "rent",
              LongTermAnnualProjectionApi.CashFlowKind.RENTAL_INCOME, bd("0"),
              LongTermAnnualProjectionApi.Source.PROJECTED)), bd("0"), request.requestedCapital(),
          request.requestedCapital().min(bd("30")), bd("70"), request.state(),
    ...
    InvestmentAnnualProjectionApi investment = request -> {
      ...
      return new InvestmentAnnualProjectionApi.AnnualProjection(request.year(), request.startValue(),
          request.externalContribution(), bd("7"), request.withdrawal().min(bd("40")), bd("67"), request.source());
    };
    var year = new RetirementSimulationOrchestrator(longTerm, investment).run(input(65, 65, bd("100"), bd("20"))).years().getFirst();
  ...
  void surplusDoesNotRefillReserveOrRequestCapital() {
    ...
    LongTermAnnualProjectionApi longTerm = longTerm(request -> {
      ...
      return new LongTermAnnualProjectionApi.PlanningProjection(request.year(), List.of(), bd("0"),
    ...
    var result = new RetirementSimulationOrchestrator(longTerm, investment).run(
        input(60, 65, bd("0"), bd("20"), bd("100"))).years().getFirst();
  ...
  void lifecycleOnlyChangesFlowsAndSpendingStartsAtRetirementBaseline() {
    LongTermAnnualProjectionApi longTerm = longTerm(request -> new LongTermAnnualProjectionApi.PlanningProjection(
        request.year(), List.of(), BigDecimal.ZERO, request.requestedCapital(), BigDecimal.ZERO,
        BigDecimal.ZERO, request.state(), LongTermAnnualProjectionApi.Source.PROJECTED));
    InvestmentAnnualProjectionApi investment = request -> new InvestmentAnnualProjectionApi.AnnualProjection(
        request.year(), request.startValue(), request.externalContribution(), BigDecimal.ZERO,
        BigDecimal.ZERO, request.startValue(), request.source());
    var years = new RetirementSimulationOrchestrator(longTerm, investment).run(
        input(64, 66, bd("100"), BigDecimal.ZERO, bd("100"))).years();
  ...
  void firstYearFundingAndInvestmentValuesFollowTheDisplayedIdentities() {
    LongTermAnnualProjectionApi longTerm = longTerm(request -> new LongTermAnnualProjectionApi.PlanningProjection(
        request.year(), List.of(
            new LongTermAnnualProjectionApi.PlannedCashFlow("rent", "rent",
                LongTermAnnualProjectionApi.CashFlowKind.RENTAL_INCOME, bd("174804"),
                LongTermAnnualProjectionApi.Source.ACTUAL),
            new LongTermAnnualProjectionApi.PlannedCashFlow("bond", "bond",
                LongTermAnnualProjectionApi.CashFlowKind.FIXED_INCOME, bd("38880"),
                LongTermAnnualProjectionApi.Source.ACTUAL)), BigDecimal.ZERO, request.requestedCapital(),
        BigDecimal.ZERO, BigDecimal.ZERO, request.state(), LongTermAnnualProjectionApi.Source.ACTUAL));
    InvestmentAnnualProjectionApi investment = request -> {
      BigDecimal annualReturn = request.startValue().multiply(request.annualReturnRate());
      return new InvestmentAnnualProjectionApi.AnnualProjection(request.year(), request.startValue(),
          request.externalContribution(), annualReturn, BigDecimal.ZERO,
          request.startValue().add(request.externalContribution()).add(annualReturn), request.source());
    };

    var year = new RetirementSimulationOrchestrator(longTerm, investment).run(
        new RetirementSimulationInput(65, 65, 2027, 65, bd("240000"), bd(".04"), BigDecimal.ZERO, 65,
            BigDecimal.ZERO, BigDecimal.ZERO, bd("710364"), bd("8614"), bd(".085"), List.of(),
            InvestmentAnnualProjectionApi.Source.PROJECTED, ExpenseProfile.EMPTY,
            LongTermAnnualProjectionApi.PlanningState.EMPTY)).years().getFirst();
  ...
  private static RetirementSimulationInput input(int age, int endAge, BigDecimal expenses, BigDecimal reserve) {
    return input(age, endAge, expenses, reserve, BigDecimal.ZERO);
  }
  private static RetirementSimulationInput input(int age, int endAge, BigDecimal expenses,
      BigDecimal reserve, BigDecimal employment) {
    return new RetirementSimulationInput(age, endAge, 2026, 65, expenses, bd(".10"), BigDecimal.ZERO,
        Integer.MAX_VALUE, employment, BigDecimal.ZERO, reserve, bd("100"), bd(".07"), List.of(),
        InvestmentAnnualProjectionApi.Source.PROJECTED, ExpenseProfile.EMPTY,
  ...
  private static LongTermAnnualProjectionApi longTerm(
      java.util.function.Function<LongTermAnnualProjectionApi.PlanningRequest,
          LongTermAnnualProjectionApi.PlanningProjection> projection) {
    return new LongTermAnnualProjectionApi() {
      @Override public AnnualProjection project(ProjectionRequest request) {
        throw new UnsupportedOperationException("legacy projection is not used");
      }
      @Override public PlanningProjection plan(PlanningRequest request) { return projection.apply(request); }
    };
  }
}
