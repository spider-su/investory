...
class CashFlowAggregationServiceTest {
  ...
  void aggregatesMonthlyAnnualAndOneOffIncomeAndExpenses() {
    var result = new CashFlowAggregationService().aggregate(new Period(date(2026, 1, 1), date(2026, 12, 31)), List.of(
        flow("rent", CashFlowDirection.INCOME, CashFlowCadence.MONTHLY, "100", date(2026, 1, 1)),
        flow("pension", CashFlowDirection.INCOME, CashFlowCadence.ANNUAL, "50", date(2026, 1, 1)),
        flow("holiday", CashFlowDirection.EXPENSE, CashFlowCadence.ANNUAL, "200", date(2026, 1, 1)),
        flow("purchase", CashFlowDirection.EXPENSE, CashFlowCadence.ONE_OFF, "300", date(2026, 6, 1))));
  ...
  void actualReplacesOnlyItsProjectedOccurrence() {
    var projected = flow("rent", CashFlowDirection.INCOME, CashFlowCadence.MONTHLY, "100", date(2026, 1, 1));
    var actual = new PlannedCashFlow("rent", "rent", CashFlowDirection.INCOME,
    ...
    var result = new CashFlowAggregationService().aggregate(
  ...
  private static PlannedCashFlow flow(String id, CashFlowDirection direction, CashFlowCadence cadence,
      String amount, LocalDate date) {
    return new PlannedCashFlow(id, id, direction, cadence, bd(amount), date, ProjectionSource.PROJECTED);
  }
