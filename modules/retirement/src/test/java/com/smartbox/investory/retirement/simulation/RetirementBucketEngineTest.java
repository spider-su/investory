...
class RetirementBucketEngineTest {
  ...
  void realEstateIsUntouchedUntilAllLiquidBucketsAreExhausted() {
    var exactlyLiquid = engine.simulate(
    ...
    var needsRealEstate = engine.simulate(
  ...
  void harvestRunsAtThresholdButNotJustBelowIt() {
    var exact = engine.simulate(
        PlanningBuckets.of(bd("0"), bd("80"), bd("1000"), bd("0"), BigDecimal.ZERO,
            bd("0.07"), bd("100"), BigDecimal.ZERO),
        BigDecimal.ZERO, BigDecimal.ZERO, policy("0.07", "1"));
    assertThat(exact.buckets().get(BucketType.BONDS).refill()).isEqualByComparingTo("20");
    assertThat(exact.buckets().get(BucketType.BONDS).expectedEndValue()).isEqualByComparingTo("100");

    var below = engine.simulate(
        PlanningBuckets.of(bd("0"), bd("80"), bd("1000"), bd("0"), BigDecimal.ZERO,
            bd("0.069999"), bd("100"), BigDecimal.ZERO),
        BigDecimal.ZERO, BigDecimal.ZERO, policy("0.07", "1"));
  ...
  void harvestShareZeroAndOneRespectEligibleGainAndBondTarget() {
    var zeroShare = engine.simulate(
        PlanningBuckets.of(bd("0"), bd("0"), bd("1000"), bd("0"), BigDecimal.ZERO,
            bd("0.10"), bd("100"), BigDecimal.ZERO),
        BigDecimal.ZERO, BigDecimal.ZERO, policy("0.07", "0"));
    assertThat(zeroShare.buckets().get(BucketType.BONDS).refill()).isZero();

    var fullShare = engine.simulate(
        PlanningBuckets.of(bd("0"), bd("0"), bd("1000"), bd("0"), BigDecimal.ZERO,
            bd("0.10"), bd("150"), BigDecimal.ZERO),
        BigDecimal.ZERO, BigDecimal.ZERO, policy("0.07", "1"));
    assertThat(fullShare.buckets().get(BucketType.BONDS).refill()).isEqualByComparingTo("100");
    assertThat(fullShare.buckets().get(BucketType.BONDS).expectedEndValue()).isEqualByComparingTo("100");
