 ...
 *
 * <p>The repository has no browser-driver dependency. This test therefore uses the existing
 * service and Thymeleaf contract layers instead of introducing a second E2E framework. It keeps
 * the browser lifecycle stages explicit and compares every scenario to the canonical simulator.
 */
class RetirementSimulationLifecycleE2ETest {
  ...
  void goldenPlanLifecyclePersistsRecalculatesAndKeepsScenariosIsolated() throws Exception {
    ...
    assertThat(scenarios).containsKeys(SimulationScenario.BASE, SimulationScenario.CONSERVATIVE,
    ...
    assertThat(simulation).contains("Scenario", "Yearly projection", "Cash flow / funding",
        "Financial outlook", "Plan status", "Projected");
    assertThat(editor).contains("1. Timeline", "2. Spending", "3. Income", "4. Events",
        "5. Reserve &amp; funding", "Total annual spending", "Save");
  ...
  private static InvestmentProfile goldenProfile() {
    var bond = new ProjectedLongTermAsset(10L, "Golden bond", LongTermAssetTypeModel.BOND,
        ...
        List.of(new ProjectedLongTermAsset.Period(LocalDate.of(2020, 1, 1), null,
            new BigDecimal("38880"), BigDecimal.ZERO, new BigDecimal("0.10"))), List.of(),
        LocalDate.of(2028, 12, 31), new BigDecimal("486000"), InterestTreatmentModel.PAY_OUT,
    ...
    var rental = new ProjectedLongTermAsset(11L, "Golden rental", LongTermAssetTypeModel.REAL_ESTATE,
        ...
        List.of(new ProjectedLongTermAsset.Period(LocalDate.of(2020, 1, 1), null,
            new BigDecimal("174803.62"), BigDecimal.ZERO, BigDecimal.ZERO)), List.of(), null, null,
    ...
    return new InvestmentProfile(1L, CurrencyType.PLN, new BigDecimal("550000"),
        ...
        new BigDecimal("3000000"), List.of(), List.of(rental, bond), new BigDecimal("174803.62"),
  ...
  private static final class InMemoryPlanStore {
    ...
    private InMemoryPlanStore() {
      ...
      when(revisions.findByIdAndSimulationPlanId(anyLong(), anyLong())).thenAnswer(invocation ->
          revisionRows.stream().filter(row -> row.getId().equals(invocation.getArgument(0))).findFirst());
