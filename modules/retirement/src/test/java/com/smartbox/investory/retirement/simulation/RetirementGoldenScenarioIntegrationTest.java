...
class RetirementGoldenScenarioIntegrationTest {
  ...
  private static void assertCheckpoints(SimulationResult result, boolean longHorizon) {
    ...
    if (longHorizon) {
      var y2052 = result.years().stream().filter(y -> y.year() == 2052).findFirst().orElseThrow();
    ...
    for (SimulationYear year : result.years()) {
      ...
      var f = year.funding();
      assertThat(f.reserveEnd())
          .isEqualByComparingTo(f.reserveStart().subtract(f.reserveWithdrawal()));
      assertThat(year.fixedIncomeEnd())
          .isEqualByComparingTo(
              year.fixedIncomeStart()
                  .add(f.capitalizedBondReturn())
                  .add(f.equityHarvestToReserve())
                  .subtract(f.longTermFunding()));
      assertThat(year.equityEnd())
          .isEqualByComparingTo(
              year.equityStart()
                  .add(year.preRetirementContribution())
                  .add(f.investmentReturn())
                  .subtract(f.investmentWithdrawal())
                  .subtract(f.equityHarvestToReserve()));
    ...
    var maturityYear =
        result.years().stream().filter(y -> y.year() == 2028).findFirst().orElseThrow();
    assertThat(money(maturityYear.bondIncome())).isEqualByComparingTo("38880");
    var postMaturity =
        result.years().stream().filter(y -> y.year() == 2029).findFirst().orElseThrow();
    assertThat(postMaturity.bondIncome()).isZero();
  }

  private static void assertYear(
