...
class CurrentYearProjectionBridgeTest {
  ...
  void currentExpectedBucketEndsAreTheFirstProjectedStarts() {
    ...
    SimulationYear priorYear = SimulationYear.bucket(
        39, 2025, false, bd("0"), bd("0"), bd("0"), bd("0"), bd("0"), bd("0"), bd("0"),
        result(BucketType.CASH, "900", "900"), result(BucketType.BONDS, "800", "800"),
        result(BucketType.EQUITIES, "700", "700"), result(BucketType.REAL_ESTATE, "600", "600"),
        BigDecimal.ZERO, BigDecimal.ZERO);
    when(simulations.simulate(eq(profile), eq(assumptions), eq(SimulationScenario.BASE), eq(2026)))
        .thenReturn(new SimulationResult(SimulationScenario.BASE, false, null, BigDecimal.ZERO,
            BigDecimal.ZERO, List.of(priorYear, currentYear)));
