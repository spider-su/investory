...
public class InvoiceValidator {
  ...
  public List<String> validate(AccountingInvoiceRecognitionService.RecognizedInvoice invoice) {
    ...
    if (blank(invoice.seller()) || blank(invoice.sellerNip()))
      issues.add("missing seller identity/NIP");
    ...
    if (invoice.netAmount() != null
        ...
        && invoice
                .netAmount()
                .add(invoice.vatAmount())
                .subtract(invoice.grossAmount())
                .abs()
                .compareTo(TOLERANCE)
            > 0) {
  ...
  public AccountingInvoiceRecognitionService.RecognizedInvoice resolveDirection(
    ...
    String own = digits(ownNip);
    String type =
        own != null && own.equals(digits(invoice.sellerNip()))
            ? "SALES_INVOICE"
            : own != null && own.equals(digits(invoice.buyerNip()))
                ? "PURCHASE_INVOICE"
                : "UNKNOWN";
