...
- Stock flow: `POST /stock/create` seeds `stocks` from open positions, `POST /stock/sync` runs quote refresh only (`MarketService.updateStocks()`), then `POST /portfolio/sync` runs `MarketService.fullPortfolioUpdate()` (quote refresh + IBKR re-pricing + history snapshot).
...
- TwelveData is called directly in `TwelveDataService` (`/quote`, `/macd`, `/rsi`, `/time_series`); key comes from `app.api.twelve-data-key`. A single shared `java.net.http.HttpClient` with a 10 s timeout drives every request; the central `get(path, params)` helper URL-encodes the parameters and wraps failures in `TwelveDataException`.
- FX rates use `ExchangeRateClient` (a thin `java.net.http.HttpClient` wrapper) -> `https://api.exchangerate.host/live`; key comes from `app.api.exchange-rate-key`.
...
- PostgreSQL + Flyway; migrations live in `src/main/resources/sql/migration/` (currently `V01.000` - `V01.007`). `V01.007__performance_indexes.sql` adds FK + hot-path B-tree indexes (e.g. `import_row_error(batch_id)`, `cash_operations(account, date)`, `opened_positions(account|symbol)`, `closed_positions(account)` and `(symbol, close_time)`, `open_positions_history(date)`) and prepares the indicator tables for proper unique snapshots (`fundamental_indicators(symbol, sync_date)`, `technical_indicators(symbol, timestamp)` — with a pre-step DELETE so the unique index applies cleanly against seed data).
...
- XTB parser is header-driven: `XtbImportService.getColumnIndexes()` depends on exact broker labels (for example `Position`, `Gross P/L`, `Open time`). The cell helper `getString(Cell)` falls back to a locale-neutral `DataFormatter` so a numeric or boolean cell never throws POI's `IllegalStateException`.
- `XtbImportService` is stateless across sheets: the per-import context (`headerRowNum`, `columnIndexes`, `account`, `currency`) is built per call in `getImportContext` and returned as a private `ImportContext` value object. Parallel imports of *different* accounts are safe; serialized writes to shared tables are still the bottleneck.
...
- `ImportOrchestratorService.importFile` checksums the upload (SHA-256); a previously-APPLIED batch with the same hash is short-circuited and returned as `duplicate=true` **without mutating** the original batch's row. The `duplicate` flag is per-upload only; `GET /import/batches/{id}` always returns `duplicate=false`.
- Failure auditing goes through `ImportBatchAuditWriter` (`@Transactional(REQUIRES_NEW)` per write): even when the parser rolls back, the FAILED `import_batch` + `import_row_error` rows still get persisted. `import_row_error.raw_payload` is capped at `ImportBatchAuditWriter.RAW_PAYLOAD_LIMIT` (8 KB) with a `[truncated to N of M bytes]` header. Parser failures bubble up as `ImportFailedException`; `ImportController` maps it to HTTP 422 and maps `IllegalArgumentException` (e.g. unknown broker) to HTTP 400.
- `MarketService.updateStocks()` intentionally rate-limits: chunks of `CHUNK_SIZE` (8) symbols with a `Duration chunkPause` between chunks (defaults to 120 s, override with `app.market.chunk-pause-ms`; tests use `0`). The pause is skipped after the last chunk and a thread interrupt stops the loop cleanly instead of throwing. Failures inside a chunk are logged and skipped (next run picks up).
...
- Useful endpoints: `POST /import/broker/{broker}`, `POST /stock/create`, `POST /stock/sync`, `POST /portfolio/sync`, `POST /portfolio/history`, `POST /currency/refresh`, `GET /portfolio/total-profit-loss`, `GET /portfolio/benchmark`, `GET /import/batches`, `GET /import/batches/{id}`, `GET /import/batches/latest`, `GET /import/batches/{id}/errors`.
