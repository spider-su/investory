...
- Use a disposable plan and record its ID. Do not delete a real plan.
- Have known source data: profile value, rental/passive income, pension, expenses, cash reserve and bond maturity, and at least one equity holding.
...
| RET-05 | Add/edit/delete plan events, including income and expense, dates inside/outside plan and invalid values. | Only valid events change affected years; delete is scoped to selected event; errors do not leave partial state. | `retirement_plan_events`; check transaction rollback on rejected input. |
| RET-06 | Use plan preview/edit; save, reload, and compare the resulting plan. | Plan boundary is explicit and projection remains deterministic. | `retirement_plans`, `retirement_plan_events`; verify expected plan and timestamps. |
| RET-07 | Mark a past year Actual/approved, attempt edit, reopen, edit, then reapprove. | Approved snapshot is immutable until explicit reopen; reopening is visible and preserves auditability. | `retirement_planning_years`; verify state transitions and no duplicate annual snapshot. |
...
| RET-09 | Inspect several Future years around bond and interest-bearing cash-reserve maturity and reserve depletion; change a long-term input, then project again. | Future is deterministic and recalculates from allowed inputs; contractual maturity behavior is correct. | Compare plan and projection facts; aggregate planning rows remain consistent. |
