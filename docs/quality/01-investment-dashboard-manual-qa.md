...
| INV-08 | Force or select an asset with manual price fallback, missing price, carry-forward FX, and missing FX in a safe environment. | Carry-forward is visible with its source date; missing FX fails closed rather than silently treating amount as target currency. | Inspect `asset_price_history`, valuation rows in `exchange_rates`, price/FX provenance and logs. Confirm no fabricated conversion/value. |
| INV-09 | Wait for or trigger scheduled market/FX refresh in a non-production environment. | Job does not overlap; successful refresh updates valuation observations only; provider failure preserves the last usable observation. | Pod logs, scheduler/job state, `asset_price_history`, `exchange_rates`; inspect purpose, restarts, OOM events and error rate. |
