...
| REC-05 | C3: check price/FX requirements for a non-base holding and deliberately missing/carry-forward fixture. | Valid provider/date/provenance is accepted; missing conversion fails closed and carry-forward remains visible. | `asset_price_history`, `exchange_rates` valuation rows, mappings, issue rows and logs. |
