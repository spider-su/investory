...
## Manual-weekly diagnostic fix

The position valuation view now emits `MANUAL_WEEKLY_CONTINUITY_REVIEW` for
selected `price_origin = 'MANUAL_WEEKLY'` rows and leaves their continuity
`expected_value` null. This preserves the selected production price and
reconstructed market value while removing manual-weekly continuity movements
from the unexplained material-mismatch population.

Local verification after replacing the diagnostic view:

| Metric | Before | After |
|---|---:|---:|
| Position valuation material mismatches | 177 | 144 |
| `OK` material rows | 33 | 0 |
| Manual-weekly review rows (all visible rows) | 0 | 166 |
| `TRADE_OBSERVATION_SELECTED` material | 144 | 144 |
...
The earlier baseline had 33 manual-weekly continuity rows. They are now
explicit review rows, not `OK` fall-through rows. No imported facts,
positions, FX values, selected production prices, reconstructed values, or
materiality thresholds were changed.

Generated: 2026-08-13T14:08:42.778377100+02:00
...
```text
count	coalesce
144	789.500000000000000000000000
```
