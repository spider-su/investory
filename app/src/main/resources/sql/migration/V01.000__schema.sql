...
CREATE TABLE investory.accounts (
    ...
    created_at      timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT ux_accounts_provider_external_account UNIQUE (provider, external_account_id)
);
...
CREATE INDEX ix_accounts_cash_only ON investory.accounts(cash_only) WHERE cash_only;
...
ALTER SEQUENCE investory.assets_id_seq INCREMENT BY 50;
...
CREATE UNIQUE INDEX ux_asset_source_symbols_source_symbol_ci
...
CREATE INDEX ix_asset_price_history_asset_date
    ON investory.asset_price_history(asset_id, price_date);
CREATE INDEX ix_asset_price_history_date
    ON investory.asset_price_history(price_date);
CREATE INDEX ix_asset_price_history_source_symbol
    ON investory.asset_price_history(source, source_symbol);
CREATE INDEX ix_asset_price_history_source_mapping
    ON investory.asset_price_history(source_mapping_id)
    WHERE source_mapping_id IS NOT NULL;
CREATE INDEX ix_asset_price_history_estimated
    ON investory.asset_price_history(asset_id, price_date)
    WHERE estimated;
CREATE INDEX ix_asset_price_history_quality
...
CREATE UNIQUE INDEX ux_exchange_rates_observation
...
CREATE UNIQUE INDEX ux_import_history_provider_sha256_attempt
...
CREATE INDEX ix_positions_source_position_id
...
CREATE UNIQUE INDEX ux_account_daily_account_date
    ON investory.account_daily(account_id, snapshot_date);
CREATE INDEX ix_account_daily_snapshot_date
...
ALTER SEQUENCE investory.account_daily_id_seq INCREMENT BY 50;
...
CREATE UNIQUE INDEX ux_benchmark_monthly_closes_symbol_month
