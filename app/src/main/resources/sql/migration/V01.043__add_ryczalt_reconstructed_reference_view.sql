-- Read-only reconstruction of monthly accounting references.
-- Invoice amounts remain the source for income/costs; bank amounts are payment evidence only.
...
WITH native_invoice_totals AS (
    SELECT
        ...
        SUM(booked_net_pln) FILTER (WHERE direction = 'INCOME') AS income_total,
        SUM(booked_net_pln) FILTER (WHERE direction = 'COST') AS costs_total,
        SUM(vat_amount) FILTER (WHERE direction = 'INCOME') AS output_vat,
        SUM(COALESCE(deductible_vat, 0)) FILTER (WHERE direction = 'COST') AS deductible_input_vat,
        COUNT(*) AS invoice_count
    FROM investory.ryczalt_invoice
    WHERE approval_status = 'APPROVED'
      AND booked_net_pln IS NOT NULL
...
periods AS (
    ...
    UNION
    SELECT profile_id, tax_period FROM obligation_totals
    UNION
    SELECT profile_id, tax_period FROM bank_evidence
)
SELECT
    p.profile_id,
    p.tax_period,
    ...
    i.invoice_source,
    CASE
        WHEN i.profile_id IS NULL OR o.profile_id IS NULL THEN 'NEEDS_REVIEW'
        ELSE 'RECONSTRUCTED'
    END AS reconstruction_status
FROM periods p
LEFT JOIN invoice_totals i
  ON i.profile_id = p.profile_id
 AND i.tax_period = p.tax_period
LEFT JOIN obligation_totals o
  ON o.profile_id = p.profile_id
 AND o.tax_period = p.tax_period
LEFT JOIN vat_adjustments v
  ON v.profile_id = p.profile_id
 AND v.tax_period = p.tax_period
LEFT JOIN bank_evidence b
  ON b.profile_id = p.profile_id
 AND b.tax_period = p.tax_period;

COMMENT ON VIEW investory.ryczalt_monthly_reconstructed_reference IS
    'Read-only monthly reconstruction from approved invoices, tax obligations, VAT adjustments, and bank payment evidence.';
