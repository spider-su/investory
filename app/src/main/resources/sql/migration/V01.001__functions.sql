...
-- Canonical valuation resolver. It reads neutral VALUATION observations only. Missing calendar
-- dates use the latest earlier observation and expose that choice as CARRY_FORWARD.
...
WITH candidates AS (
    SELECT er.rate, er.source, er.method,
           COALESCE(er.source_rate_date, er.rate_date) AS source_rate_date,
           er.rate_date, 0 AS direction_priority
    ...
    UNION ALL
    SELECT 1 / er.rate, er.source, er.method,
           COALESCE(er.source_rate_date, er.rate_date), er.rate_date, 1
    FROM investory.exchange_rates er
    WHERE er.purpose = 'VALUATION'
    ...
    ORDER BY rate_date DESC, direction_priority
    LIMIT 1
)
SELECT p_source_currency, p_target_currency,
       ...
       CASE WHEN p_source_currency = p_target_currency THEN 'SAME_CURRENCY'
            WHEN selected.rate_date < p_valuation_date THEN 'CARRY_FORWARD'
            ELSE selected.method END,
       CASE WHEN p_source_currency = p_target_currency THEN 'SAME_CURRENCY' ELSE selected.source END,
       CASE WHEN p_source_currency = p_target_currency THEN p_valuation_date
            ELSE selected.source_rate_date END,
...
COMMENT ON FUNCTION investory.resolve_fx_rate(date, varchar, varchar) IS
    'Canonical valuation FX resolver. Uses only VALUATION observations and carries the latest earlier observation forward without materializing copied rows.';
...
-- A reconstructed valuation must never consume a normalized-price materialization
-- that predates the canonical historical-price refresh.  The old function only
-- refreshed the reconstruction itself, so a stale normalized price/currency pair
-- could survive into reconstructed values.
CREATE OR REPLACE FUNCTION investory.refresh_reconstructed_position_daily()
RETURNS void
LANGUAGE plpgsql
AS $$
