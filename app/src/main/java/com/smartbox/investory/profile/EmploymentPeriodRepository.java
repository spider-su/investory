...
public class EmploymentPeriodRepository {
  ...
  public List<EmploymentPeriod> findAll(long profileId) {
    return jdbc.query(
        "SELECT id, employment_type, date_from, date_to, qualifies_as_primary_social_insurance FROM investory.employment_period WHERE profile_id = ? ORDER BY date_from, id",
        (rs, row) ->
  ...
  public EmploymentPeriod save(long profileId, EmploymentPeriod period) {
    Long id =
        period.id() == null
            ? jdbc.queryForObject(
                "INSERT INTO investory.employment_period(profile_id, employment_type, date_from, date_to, qualifies_as_primary_social_insurance) VALUES (?, ?, ?, ?, ?) RETURNING id",
                Long.class,
                profileId,
                period.type().name(),
                period.from(),
                period.to(),
                period.qualifiesAsPrimarySocialInsuranceTitle())
            : period.id();
    if (period.id() != null)
      jdbc.update(
          "UPDATE investory.employment_period SET employment_type = ?, date_from = ?, date_to = ?, qualifies_as_primary_social_insurance = ? WHERE id = ? AND profile_id = ?",
          period.type().name(),
          period.from(),
          period.to(),
          period.qualifiesAsPrimarySocialInsuranceTitle(),
