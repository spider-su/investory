  ...
  void freezesKnownMonthlyComparisonMatrix() {
    ...
    expected.forEach(
        (month, monthExpected) -> {
          ...
          assertThat(status(snapshot, "REVENUE")).as(month + " revenue").isEqualTo(monthExpected.revenue());
          assertThat(status(snapshot, "RYCZALT")).as(month + " ryczalt").isEqualTo(monthExpected.ryczalt());
