...
import com.example.demo.services.openai.PortfolioContextService;
...
public class PortfolioCommandService {
    ...
    String execute(PortfolioCommand command) {
        ...
        return switch (command) {
            ...
            case POSITIONS -> section(context, "Current positions", List.of("Realized P/L", "Realized by Currency"), 1800);
            ...
            case REPORT -> "No stored deterministic report is available yet. Scheduled reports are delivered through Telegram; historical report storage should be added before enabling /report.";
            case HELP -> help();
        };
    }
    ...
    private String allocation(String context) {
        List<String> lines = new ArrayList<>();
        String unrealized = sectionValue(context, "Unrealized by Currency", List.of("Realized by Currency"), 900);
        String realized = sectionValue(context, "Realized by Currency", List.of("Current positions"), 900);
        if (unrealized != null) {
        ...
        if (lines.isEmpty()) {
            lines.add("Asset-class and position-weight allocation is not exposed by the current dashboard context.");
        }
    ...
    private String risk(String context) {
        List<String> lines = new ArrayList<>();
        String positions = sectionValue(context, "Current positions", List.of("Realized P/L", "Realized by Currency"), 1200);
        if (positions != null) {
            lines.add("Current positions: " + positions);
        }
        lines.add("Deterministic concentration, drawdown, volatility and allocation-limit metrics are not yet calculated by Investory.");
    ...
    private String section(String context, String start, List<String> ends, int maxLength) {
        ...
        return value == null ? "No " + start.toLowerCase(Locale.ROOT) + " data was found." : start + ":\n" + value;
    }

    private static void addMetric(List<String> lines, String context, String outputLabel, String sourceLabel) {
    ...
    static String findValue(String context, String label) {
        Pattern pattern = Pattern.compile(
                "(?i)(?:^|\\s)" + Pattern.quote(label) + "\\s*:?\\s*(.{1,80}?)(?=\\s+(?:Balance|Cash|ROI|Profit|After tax|Unrealized P/L|Realized P/L|Dividends|Current positions|Unrealized by Currency|Realized by Currency)\\b|$)");
        Matcher matcher = pattern.matcher(context);
        return matcher.find() ? clean(matcher.group(1), 100) : null;
    }

    static String sectionValue(String context, String startLabel, List<String> endLabels, int maxLength) {
