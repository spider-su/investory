 ...
 * persist a {@code FAILED} batch, Spring rolled the whole transaction back and the audit
 * trail was lost. Each method here runs in {@link Propagation#REQUIRES_NEW}, so the audit
 * rows survive even when the surrounding parser transaction rolls back.
 */
@Component
...
public class ImportBatchAuditWriter {
    ...
    public ImportHistory startBatch(BrokerType broker,
    ...
    public ImportHistory finalizeFailed(Long batchId, String message, byte[] rawPayload) {
        ...
        batch.setErrorMessage(payloadPreview == null || !isTextLike(rawPayload) ? message : message + "\n" + payloadPreview);
        batch.setFinishedAt(ZonedDateTime.now());
