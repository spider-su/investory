...
class XtbAccountFundingCalculatorTest {
  ...
  void knownXtbUsdReconciliationAdjustmentsAreRepresentedExactly() {
    CashOperation firstOut = transfer(1L, 51499241L, -801.47, "Transfer from 51993106 to 51499241");
    CashOperation firstIn = transfer(2L, 51499241L, 801.47, "Transfer from 51993106 to 51499241");
    CashOperation secondOut =
        transfer(3L, 51993106L, -995.31, "Transfer from 51499241 to 51993106");
    CashOperation secondIn = transfer(4L, 51993106L, 995.31, "Transfer from 51499241 to 51993106");
  ...
  void nonXtbAndNonUsdAccountsAreUnaffected() {
    CashOperation operation = transfer(1L, 100L, -1000, "Transfer from 100 to 200");
    CashOperation pair = transfer(2L, 100L, 1000, "Transfer from 100 to 200");
    CashOperation eurOperation = transfer(3L, 200L, -500, "Transfer from 200 to 300");
    CashOperation eurPair = transfer(4L, 200L, 500, "Transfer from 200 to 300");
    eurOperation.setCurrency(CurrencyType.EUR);
    eurPair.setCurrency(CurrencyType.EUR);
    Account ibkr = account(100L, "IBKR", CurrencyType.USD);
    Account xtbEur = account(200L, "XTB", CurrencyType.EUR);
  ...
  private Map<Long, Double> calculate(CashOperation... operations) {
    return calculator.calculate(
        List.of(operations), Map.of(100L, account(100L, "XTB", CurrencyType.USD)));
  }

  private static Account account(Long id, String provider, CurrencyType currency) {
  ...
  private static CashOperation transfer(long id, long accountId, double amount, String comment) {
